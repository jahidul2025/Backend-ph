...
const initiatePayment = async (appointmentId: string, user: IRequestUser) => {
    ...
    if (!appointmentData.payment) {
        throw new AppError("Payment data not found for this appointment", status.NOT_FOUND);
    }

    if (appointmentData.payment?.status === PaymentStatus.PAID) {
        throw new AppError("Payment already completed for this appointment", status.BAD_REQUEST);
    };
